...
def train(epoch, train_data):
    return ContainerStep(
        ...
        command="bash artifact_example/shells/train.sh {{epoch}} {{train_data}} {{model_path}} {{train_model}}",
    )
...
def validate(data, model):
    return ContainerStep(
        ...
        command="bash artifact_example/shell/validate.sh {{model}}", 
    )

@Pipeline(name="artifact_example", docker_env="nginx:1.7.9")
...
if __name__ == "__main__":
    ...
    ppl.compile(save_path=__file__ + ".yaml")